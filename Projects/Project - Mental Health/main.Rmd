...
```{r depression 2020}
plot_depression_2020 <- depression_map_ctgs |> 
ggplot() +
...
```{r depression 2020 dmv}
depression_map_ctgs |> filter(STATE_NAME %in% c("Maryland", "Virginia", "District of Columbia")) |> 
ggplot() +
  geom_sf(aes(fill = val_depression)) + 
  coord_sf(datum = NA) + 
  ggthemes::theme_map() + 
...
dep_improv_20 <- dep_val_20 - dep_val_19
dep_improv_21 <- dep_val_21 - dep_val_20
dep_accel <- 0.5 * (dep_improv_21 - dep_improv_20)
```
