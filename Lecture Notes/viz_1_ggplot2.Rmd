...
- better to think of the data and code as what’s real, together with the text you write
...
```{r increase mpg point size globally, out.width="80%"}
g + geom_point(
  aes(x = displ, y = hwy, col = class), size = 3)
```
...
Suppose we use shapes for classes and colors for manufacture years. What went wrong?

```{r, out.width="70%", fig.align='center'}
ggplot(data = subset(mpg, class %in% class_list)) +
  geom_point(aes(x = displ, y = hwy, 
                 shape = class, color = year), 
  size = 3)
```
...
Note: factors are stored as **integers** (not as characters), and a attribute giving the levels
...
Some aesthetic options for `geom_point` include:
...
Looks more spread out, but anything to fix/improve?
...
```{r}
p <- ggplot(df, aes(x,y)) + 
  geom_label(aes(label = job), size = 8)
p
```
...
\small
- `theme(...)` shrinks the title. More on `theme()` later
...
```{r, out.width="55%"}
ggplot(df, aes(x,y)) + geom_text(aes(label = job), size = 7, 
...
```{r}
p + geom_line(
  aes(color = country, linetype = country), 
  linewidth = 1.5) +
...
p <- ggplot(
  ...
  mapping = aes(x = partyid)) + geom_bar() 
p
```
...
```{r, out.width="70%"}
ggplot(data = subset(gss_sm, partyid != "NA"),
  mapping = aes(x = partyid)) + 
  geom_bar(aes(fill = bigregion), position = "fill")
```
