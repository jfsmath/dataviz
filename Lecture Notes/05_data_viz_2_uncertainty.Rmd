...
## We can use `size` to weigh the points:
...
p1 <- ggplot(
  ...
  geom_point(aes(size = pop), color = "darkgreen", alpha = 0.6) + 
...
```
\normalsize
However, is there anything inconsistent between the plot and the regression model?

## Adjust the weight inside the statistical `geoms`

- Previous, the regression model is **unweighted**: a country with population 100 is treated as the same with a country of population 1 billion!

- To fix this, use `geom_smooth(aes(weight = pop))`
...
Minor graphical difference, but scientifically important nonetheless!!
...
```{r, out.width="60%"}
p <- ggplot(
  subset(gap2, year %in% c(1957, 1982, 2007)), aes(x = as.factor(year), y = meanLE)) +
