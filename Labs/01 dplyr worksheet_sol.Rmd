...
cars |>
  ...
  filter(speed > 15) |>
  mutate(
    dist_per_speed = dist/speed
  ) |>
  arrange(desc(dist_per_speed)) |>
  summarize(
    avg_speed = mean(speed), mean(dist), mean(dist_per_speed)
  )
```
...
```{r}
# placeholder vector
v <- rep(NA, length(cars$speed))

# indices for slow medium fast
ind_slow <- cars$speed <= 10
ind_medium <- (cars$speed > 10) & (cars$speed <= 20)
ind_fast <- cars$speed > 20
...
cars |> 
  mutate(
    speed_group2 = case_when(
      speed <= 10 ~ "slow",
      speed <= 20 ~ "medium",
      TRUE ~ "fast")) %>%
  {.$speed_group != .$speed_group2} |> sum(na.rm= T)
```
